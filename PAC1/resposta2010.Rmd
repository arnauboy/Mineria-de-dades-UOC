...
Veiem com a la primera fila del resultat ens indica que disposem de **30296 tuples** i **47 atributs** o característiques.
...
En aquest apartat, prepararem les dades perquè se'ls puguin aplicar les eines que construiran el model. Les dades resultants d'aquesta etapa hauran de ser de prou qualitat, necessàries i s'hauran de trobar en un format adequat.

### Neteja

En aquest apartat tractarem les dades nul·les o absents.
...
Un cop comprovat que no tenim cap valor nul, cal veure si existeixen camps buits.

```{r echo=TRUE, message=FALSE, warning=FALSE}
colSums(accidentData=="")
```

En aquest resultat, sí que podem veure com el camp TWAY_ID2 té **23201** camps buits. Aquest camp fa referència a la via de trànsit l'any 2004, per el que és una dimensió geogràfica. Els valors en blanc poden generar problemes en el funcionament de certs algorismes durant l'etapa de construcció del model. Des del punt de vista de la gestió de característiques, cal comprovar el percentatge de valors en blanc, i així decidir què fem amb aquest camp.

```{r echo=TRUE, message=FALSE, warning=FALSE}
sum(accidentData$TWAY_ID2=="")/30296*100
```

Observem que el percentatge de valors en blanc és d'aproximadament el 77%. Si aquesta dimensió resultés d'interès, caldria plantejar-se si realment es vol tenir en compte donat l'alta proporció de dades buides. Tal com hem indicat a l'etapa d'exploració de les dades, aquesta dimensió no forma part del subconjunt d'interès, per la qual cosa serà eliminada juntament amb la resta de dades prescindibles.
...
```{r}
n = c("FATALS","DRUNK_DR","VE_TOTAL","VE_FORMS","PEDS","PERSONS","STATE","MONTH","HOUR","WEATHER")
...
D'aquest primer resum, ja podem extreure coneixement molt valuós.

+ A tots els accidents hi ha com a mínim una víctima mortal.
+ 0,328 conductors beguts per accident.
+ 1,515 vehicles implicats per accident
+ 2,29 persones implicades per accident

#### Histogrames

Pot resultar interessant veure les dades mitjançant histogrames i fer algunes **observacions**. Començarem amb el nombre de morts i de vehicles implicats.
...
Observem com hi ha molta diferència en el nombre d'accidents per estat, molt probablement conseqüència de la quantitat de gent que hi viu a cadascun d'ells.

**MONTH**

Fent una ullada al gràfic es pot veure que hi ha una major quantitat d'accidents durant els mesos a l'interval [5 (maig), 12(desembre)].
...
**HOUR**
Veiem com augmenta el nombre d'accidents a mesura que passa el dia. Veiem com hi ha hores amb valor 99. Això és a causa del fet que es va decidir afegir un 99 a tots els casos o no es sabia l'hora de l'accident.

**DRUNK_DR**
El nombre de conductors beguts va d'1 a 4.
...
n = c("DRUNK_DR","VE_TOTAL","PERSONS","PEDS","WEATHER") 
accidentDataAux= accidentDataReduced %>% select(all_of(n))
...
+ Veiem com amb totes les variables, menys la de condició climàtica (WEATHER), augmenta el número de morts a mesura que ho fa la variable.

+ La variable de WEATHER, al ser codis de condicions climàtiques, no mostra relació amb la quantitat de víctimes mortals.

Utilitzem les columnes que ens interessen per fer la matriu i la visualitzarem utilitzant la funció corrplot.

```{r echo=TRUE, message=FALSE, warning=FALSE}
if(!require("corrplot")) install.packages("corrplot"); library("corrplot")
n = c("FATALS","DRUNK_DR","VE_TOTAL","VE_FORMS","PEDS","PERSONS","STATE","MONTH","HOUR")
...
Aprofundirem en la correlació entre persones begudes i víctimes mortals, ja que sembla que estiguin menys relacionades del que es podia pensar en un principi. Utilitzarem el mètode **kendall**.
...
Després d'observar el grau de correlació de Kendall, podem dir que el nombre de persones mortes augmenta quan el nombre de conductors beguts és més alt, pero la correlació no és d'un grau tan alt com podiem pensar en un principi. De fet, és tan baix que podria no tenir importància a efectes pràctics.

## Transformació de les dades
Com hem vist anteriorment, tenim alguns atributs que estan tan correlacionats que aporten la mateixa informació, com és el cas de VE_TOTAL amb VE_FORMS. En aquests casos, cal eliminar una de les dues característiques. 
...
Veiem que la major part dels accidents es van produir els tres darrers trimestres de l'any.

Anem a veure com es relaciona aquesta nova variable discretitzada amb els conductors beguts.

```{r echo=TRUE, message=FALSE, warning=FALSE}
files=dim(accidentDataAux)[1]
ggplot(data=accidentDataAux[1:files,],aes(x=DRUNK_DR,fill=trimestre))+geom_bar()+ggtitle("Relació entre les variables conductor begut i la condició climàtica")+labs(x="Nombre de conductors beguts implicats",y="Condició climàtica")
```

L'única observació que podem fer és la conservació de la proporció de cada trimestre en cadascuna de les columnes.
...
n = c("FATALS","DRUNK_DR","VE_TOTAL","PEDS","PERSONS","MONTH")
...
Com a resultat d'aquest procés de normalització, tenim tots els camps normalitzats per la diferència dins la variable **accidentData_nor**.

## Procés de PCA 

L'anàlisi de components principals (PCA) és una tècnica que ens permetrà treballar amb noves característiques anomenades components, independents entre si. Aquest sistema permet recollir millor la variabilitat perquè està més ben adaptat al joc de dades. A la pràctica, el que farem serà afegir al joc de dades original aquests nous components principals que recullin una variabilitat més alta.
...
Veiem com el primer i segon component principal són els que més expliquen la variabilitat del conjunt de dades, representant el 0.2947 (29.47%) i el 0.1953 (19.53%) respectivament. L'histograma següent mostra de forma visual el pes de cada dimensió sobre el total de dades.
...
Veient aquestes variàncies, apliquem el mètode Kàiser i ens quedem amb els components 1 i 2.
...
```{r}
head(var$cos2[,1:2],11)
```
```{r}
corrplot(var$cos2[,1:2], is.corr=FALSE)
```

Veiem com MONTH no aporta informació a cap dels dos components principals. Això es degut a que està altament relacionada amb la 3a.

```{r}
corrplot(var$cos2[,2:3], is.corr=FALSE)
```

A continuació veurem un diagrama de barres que representa amb columnes la qualitat de representació dels dos components principals
...
Podem observar com la variable MONTH es troba al centre de la trama, pel que no es gaire representativa de la qualitat dels dos components principals. D'altra banda, atributs com PERSONS, VE_TOTAL o DRUNK_DR es troben molt a prop del cercle de correlacions.

### Contribució

A continuació, observem la contribució (expresada en %) de cadascuna de les característiques als components principals. Aquelles que no contribueixin a la variablitat de les primeres dimensions es podran eliminar per simplificar l'anàlisi.

```{r}
head(var$contrib[,1:2],11)
```
...
La primera observació que es pot fer i que es veu a simple vista, és que MONTH no contribueix gaire als dos components principals.
...
Un exemple de **correlació positiva** son el vehicles totals (VE_TOTALS) i el nombre de persones implicades (PERSONS). D'aqui podem extreure la informació que quants més vehicles implicats en l'accident, més persones implicades.

Un exemple de correlació negativa és el nombre de vianants (PEDS) i el nombre de conductors beguts (DRUNK_DR). No sembla que hi hagi una relació de causalitat entre aquestes dues variables, pel que aquesta correlació negativa cal prendre-la simplement com una relació estadística, molt probablement provocada per altres dades.

VE_TOTAL i PERSONS contribueixen molt a la dimensió 2, mentre que DRUNK_DR i PEDS ho fan més a la 1.

Aquests nous components o dimensions obtinguts mitjançant el PCA es podrien utilitzar com a indexos de perillositat d'un accident. 

## Interpretació dels resultats

Hem estudiat un conjunt de dades sobre **accidents de trànsit**, extret del departament de transport dels Estats Units durant l'any **2010**. Aquestes dades contenen informació com el nombre de víctimes mortals, conductors beguts, vehicles implicats, vianants, dates, condicions climàtiques, dimensions geogràfiques, i així fins a 47 atributs. En total, en aquestes dades hi ha enregistrats 30296 accidents mortals.

En el procés de neteja hem vist com en general eren unes dades bastant completes, amb excepcions com ha sigut el cas del camp **TWAY_ID2**. A l'inici s'ha establert que aquest no era un dels camps d'interès, per la qual cosa era una dada prescindible. Hem aprofitat per **reduir la dimensionalitat** del conjunt de dades amb el que estàvem treballat, i hem dut a terme tota l'activitat amb el subconjunt de característiques que hem indicat que ens interessaven a l'inici.

Durant la visualització de les dades de les quals disposàvem, hem vist com tots els accidents tenien com a mínim una víctima mortal. Altres observacions interessants són les diferències respecte a nombre d'accidents entre estats, molt probablement provocades per les diferències demogràfiques entre aquests i que la majoria d'accidents s'han produït als tres últims trimestres de l'any. Aquesta última observació s'ha dut a terme **discretitzant els mesos en trimestres** i eliminant aquelles tuples en les quals no es tenia coneixement del mes.

Hem intentat aprofundir en la relació entre conductors beguts i trimestre, amb la hipòtesi que podien estar relacionades d'alguna manera. No hem pogut extreure gaire informació dels gràfics que hem generat. També hem generat gràfics que representen la relació entre el nombre de víctimes mortals amb les demés característiques d'interès. Hem vist que estava molt relacionat amb el nombre de vianants, de vehicles totals, i de persones. També tenia relació amb el nombre de conductors beguts, però en un grau molt més baix.

En últim lloc, hem generat una **nova dimensió mitjançant la tècnica del PCA** que podria ajudar-nos a valorar l'**índex de perillositat i de risc de mortalitat** en un accident de trànsit, ja que està relacionat amb els vehicles implicats, el nombre de persones i les víctimes mortals.